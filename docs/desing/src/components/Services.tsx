...
export default function Services() {
  ...
  return (
    <section id="servicios" className="relative scroll-mt-24 overflow-hidden py-24 lg:py-32">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -left-52 top-1/3 -z-10 h-[34rem] w-[34rem] rounded-full bg-[radial-gradient(circle,rgba(232,184,191,0.28),transparent_65%)] blur-3xl animate-drift"
      />

      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6">
        <div className="max-w-3xl">
          <Reveal>
            <SectionLabel>Servicios de autor</SectionLabel>
          </Reveal>
          <Reveal delay={90}>
            <h2 className="mt-5 font-display text-[2.1rem] font-light leading-[1.08] tracking-[-0.025em] text-ink-950 sm:text-5xl">
              Rituales capilares diseñados para{" "}
          ...
          <Reveal delay={170}>
            <p className="mt-5 text-[1.02rem] leading-relaxed text-ink-700">
              Nada se aplica sin antes leer tu fibra capilar. Cada servicio empieza con un
              diagnóstico digital gratuito y termina con una rutina personalizada para casa.
            </p>
          </Reveal>
        </div>
        ...
        <div className="mt-14 grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
          ...
          <div className="flex flex-col gap-3">
            {services.map((s, i) => {
              ...
              return (
                <Reveal key={s.name} delay={i * 100} direction="left">
                  <button
                    type="button"
                    onClick={() => setActive(i)}
                    ...
                    className={cn(
                      ...
                      isActive
                        ? "border-gold-400/60 bg-white shadow-[0_24px_55px_-30px_rgba(27,16,22,0.6)]"
                        : "border-ink-900/8 bg-white/50 hover:border-gold-400/40 hover:bg-white/80",
                    )}
                  >
                    <span
                      className={cn(
                        "absolute inset-y-0 left-0 w-[3px] origin-top bg-gradient-to-b from-gold-300 via-gold-500 to-gold-600 transition-transform duration-500",
          ...
          <Reveal direction="right" delay={180} className="lg:sticky lg:top-28">
            <div className="relative overflow-hidden rounded-[2rem] border border-white/60 shadow-[0_44px_90px_-46px_rgba(27,16,22,0.72)]">
              {services.map((s, i) => (
                <img
                  key={s.name}
                  src={s.image}
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 sm:p-8">
                <div key={current.name} className="reveal is-visible">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="group mt-5 inline-flex items-center gap-2 rounded-full bg-cream-50 px-5 py-3 text-[0.82rem] font-semibold text-ink-950 transition-all duration-300 hover:-translate-y-0.5 hover:bg-gold-300"
                  >
